...
# --- Configurazione Gemini per Estrazione ---
...
with tab_dispensa:
  ...
  with col_upload:
    ...
    extracted_data = {}
    if (
        st.button("Estrai Valori")
        and api_key
        and (uploaded_img or url_input)
    ):
      genai.configure(api_key=api_key)
      model = genai.GenerativeModel("gemini-1.5-flash")

      prompt = """
            Estrai i valori nutrizionali per 100g dall'immagine/testo nel seguente formato JSON puro senza markdown:
            {
              "nome": "Nome Alimento",
              "kj": 0.0,
              "kcal": 0.0,
  ...
  with col_manual:
    ...
    with st.form("form_alimento"):
      nome = st.text_input(
          "Nome Alimento", value=extracted_data.get("nome", "")
      )
      categoria = st.selectbox(
          "Categoria Dispensa", ["COLAZIONE", "PRANZO", "SPUNTINO", "CENA"]
      )

      c1, c2 = st.columns(2)
      kj = c1.number_input(
          "Energia (kJ)", value=float(extracted_data.get("kj", 0.0))
      )
      kcal = c2.number_input(
          "Energia (kcal)", value=float(extracted_data.get("kcal", 0.0))
      )

      c3, c4 = st.columns(2)
      grassi = c3.number_input(
          "Grassi (g)", value=float(extracted_data.get("grassi", 0.0))
      )
      saturi = c4.number_input(
          "di cui acidi grassi saturi (g)",
          value=float(extracted_data.get("saturi", 0.0)),
      )

      c5, c6 = st.columns(2)
      carboidrati = c5.number_input(
          "Carboidrati (g)",
          value=float(extracted_data.get("carboidrati", 0.0)),
      )
      zuccheri = c6.number_input(
          "di cui zuccheri (g)",
          value=float(extracted_data.get("zuccheri", 0.0)),
      )

      c7, c8, c9 = st.columns(3)
      fibre = c7.number_input(
          "Fibra alimentare (g)", value=float(extracted_data.get("fibre", 0.0))
      )
      proteine = c8.number_input(
          "Proteine (g)", value=float(extracted_data.get("proteine", 0.0))
      )
      sale = c9.number_input(
          "Sale (g)", value=float(extracted_data.get("sale", 0.0))
      )
...
with tab_piano:
  ...
  selected_day = st.selectbox("Seleziona Giorno da modificare/visualizzare", giorni)

  st.markdown(f"### Dettaglio: {selected_day}")
  cols = st.columns(4)
  for idx, pasto in enumerate(pasti):
    with cols[idx]:
      ...
      scelta = st.multiselect(
          f"Alimenti per {pasto}", items, key=f"{selected_day}_{pasto}"
      )
